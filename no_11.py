...
page = st.sidebar.selectbox('Select Page',['Home','Grafik','Pencarian','About'])

model = pickle.load(open('model_prediksi_harga_mobil.sav', 'rb'))

if page=='Home':
    st.title('Prediksi Harga Mobil')
    st.image('gambar.jpg',width=400)
    st.header('Dataset')
    ...
    df1 = pd.read_csv('CarPrice.csv')
    highwaympg = st.slider("Highway ", 0,10000000)
    curbweight = st.slider("curbweight ", 0,10000000)
    horsepower = st.slider("horsepower ", 0,10000000)
    ...
    if st.button('Prediksi'):
        ...
        st.success(harga_mobil_formatted)
elif page=='About':
    st.title('by Andhika Wahyu Agustian')
    st.markdown('223307090')